...
if get(g:, 'cpp_member_variable_highlight', 0)
    syn match   cCustomMemVar "\(\.\|->\)\s*\<\h\w\+\>"
    hi def link cCustomMemVar cFunction
endif

"  Highlight POSIX functions.
if get(g:, 'cpp_posix_standard', 0)
    ...
    hi def link cPOSIXFunction cFunction
endif
".}}}
...
syn keyword    cAnsiName    CNX16 SCNXFAST16 SCNXLEAST16
syn keyword    cAnsiName    CNX32 SCNXFAST32 SCNXLEAST32
syn keyword    cAnsiName    CNX64 SCNXFAST64 SCNXLEAST64
syn keyword    cAnsiName    CNX8 SCNXFAST8 SCNXLEAST8
syn keyword    cAnsiName    CNXMAX SCNXPTR
...
syn keyword cSpecialKeyword break continue return
syn keyword cGotoKeyword    goto
syn keyword cExtLangKeyword asm fortran

" Preprocess reserved keywords
syn region cPreCondit  start="^\s*\zs\(%:\|#\)\s*\(if\|ifdef\|ifndef\|elif\)\>" skip="\\$" end="$" keepend
                       \ contains=cComment,cCommentL,cCppString,cCharacter,cCppParen,cParenError
                       \ ,cNumbers,cCommentError,cSpaceError,cOperator,cCustomOperator
".}}}

"  Additional optional highlighting ----------------------------------------{{{
" User custom macro name
syn match   defineLine "\v^\s*#\s*define.*$" contained contains=cNumber,cFloat,cBoolean,cComment
            \ ,cRepeat,cLabel,cType,cStructure,cStorageClass,cTypedef,cConditional
            \ ,cDelimiter,cBraces,defineKeyword,userMacroName,cCustomOperator
            \ ,cCustomFunc,cCustomMemVar,cSpecialKeyword,cCustomClassName,cCustomClass
            \ ,cSpecialDelimiter
syn region  userMacroName start=/\s/ms=s+1 skip=/\v\/\*.*/ end=/\s/me=e-1 contained oneline
            \ contains=cDelimiter,cSpecialDelimiter,cCustomOperator,cNumber,cFloat
syn match   defineKeyword "\v^\s*#\s*define" contained
hi def link userMacroName cMacro
...
" Structure typedef name
syn region userStructAlias start=/\%(}\)\@<=/ms=s+1 end=/;/me=e-1 oneline contains=cBraces
hi def link userStructAlias cStructDeclare

" Goto labels
syn match gotoLabelDef "^\s*\zs\(\(default\|case\|\W\|;\)\@!.\)*:$"
hi def link gotoLabelDef cGotoKeyword
".}}}

" Operator and delimiter settings ------------------------------------------{{{
" Operators
if get(g:, 'cpp_operator_highlight', 0)
    syn match cCustomOperator    "\v\*[^/]"me=e-1
    syn match cCustomOperator    "\v/[^/*]"me=e-1
    syn match cCustomOperator    "\v[^*]/$"ms=s+1
    syn match cCustomOperator    "\v\+"
    syn match cCustomOperator    "-[^>]"me=e-1
    syn match cCustomOperator    "\v\%"
    syn match cCustomOperator    "\v\?"
    syn match cCustomOperator    "\v\m&"
    syn match cCustomOperator    "\v\|"
    syn match cCustomOperator    "\v\^"
    syn match cCustomOperator    "\v\~"
    syn match cCustomOperator    "\v!"
    syn match cCustomOperator    "\v\m<"
    syn match cCustomOperator    "[^-]>"ms=s+1
    syn match cCustomOperator    "\v:"
    syn match cCustomOperator    "\v\*\="
    syn match cCustomOperator    "\v/\="
    syn match cCustomOperator    "\v\+\="
...
hi def link cBraces cDelimiter

syn match cStatementDelimiter    "[,;]"
hi def link cStatementDelimiter cSpecialDelimiter
".}}}
